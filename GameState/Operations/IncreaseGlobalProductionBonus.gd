...
func execute(executionState:ExecutionState):
	...
	if maxAfford * increaseBonusAmount > 0 && executionState.gameState.globalProductionBonuses.has("rocks") && executionState.gameState.globalProductionBonuses["rocks"] <= 0:
